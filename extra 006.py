...
class Plot(pygame.sprite.Sprite):
    def __init__(self, filename):
        ...
        self.image = pygame.image.frombuffer(self.PILimage.tostring(), self.PILimage.size, 'RGB')
        # self.image = pygame.Surface(self.PILimage.size) #förbereder en pygame image surface för visning i fönster, den skall fyllas med data
        ...
        self.resImages = []
        self.resData = []
        for level in range(self.resLevels):
            self.resImages.append(Image.new('RGB', (self.PILimage.size[0], self.PILimage.size[1])))
            self.resData.append(self.resImages[level].load())
        # print self.resData[0][0,0]
    ...
    def setLevel(self, level):
        self.image = pygame.image.frombuffer(self.resImages[level].tostring(), self.resImages[level].size, 'RGB') # frombuffer returnerar en surface
        self.rect = self.image.get_rect() #måste finnas en rect annars klagar pygame

    def gatherData(self):
        timer = time()
        print "gathering..."
        for row in range(self.PILimage.size[1]): #en rad i taget
            for col in range(self.PILimage.size[0]): # här sker varje pixel
                cropsize = 3
                for level in range(self.resLevels):
                    csize = (cropsize-1)/2
                    ...
                    cropB = min(self.PILimage.size[1], row+csize)                #0,48s
                    crop = self.PILimage.crop((cropL, cropT, cropR, cropB))         #1,89s
                    cropdata = list(crop.getdata())                                       #3,26s
                    print cropdata
                    # meanA = numpy.mean(cropdata) #test mean time
                    # meanA = numpy.mean(cropdata, axis=0)
                    # meanB = numpy.mean(meanA, axis=0)
                    # meanList = tuple(meanB)
                    # meanListInt = (int(meanList[0]), int(meanList[1]), int(meanList[2]))

                    # self.resolver[col][row][level] = list(numpy.mean(numpy.mean(crop, axis=0), axis=0))
                    # print self.resData[0][0,0]
                    # print meanList

                    # self.resData[level][col,row] = meanListInt
                    # self.resData[level][col,row] = (100,200,39)
                    cropsize *= 3
    ...
    while going:
        ...
        for event in pygame.event.get():        
            if event.type == QUIT:
                ...
                plot.setLevel(int(event.unicode))
